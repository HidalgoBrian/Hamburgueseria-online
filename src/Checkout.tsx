...
export default function DeliveryCheckout({
  ...
  const radius = Number(
    settings.delivery_radius_km || demoSettings.delivery_radius_km,
  );
  ...
  const verifyAddress = async () => {
    ...
    if (!street || !locality) {
      setVerification({
        status: "invalid",
        message:
          "Ingresá calle, altura y localidad. Ejemplo: París 1725 Isidro Casanova.",
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `https://apis.datos.gob.ar/georef/api/v2.0/direcciones?${query}`,
      );
      if (!response.ok) throw new Error("Georef no disponible");
      const data = (await response.json()) as { direcciones?: GeorefAddress[] };
      const candidate = data.direcciones?.find(
        (entry) => entry.ubicacion.lat != null && entry.ubicacion.lon != null,
      );
      if (
        !candidate ||
        candidate.ubicacion.lat == null ||
        candidate.ubicacion.lon == null
      ) {
        setVerification({
          status: "invalid",
          message:
            "No pudimos ubicar esa dirección. Revisá la calle, altura y localidad.",
        });
        return;
      }
      const distance = distanceInKm(
        storeLat,
        storeLng,
        candidate.ubicacion.lat,
        candidate.ubicacion.lon,
      );
      setCustomer((current) => ({
        ...current,
        normalized_address: candidate.nomenclatura,
        delivery_lat: candidate.ubicacion.lat!,
        delivery_lng: candidate.ubicacion.lon!,
      ...
      setVerification(
        distance <= radius
          ? {
              status: "valid",
              message: `Dirección dentro del área de entrega: ${distance.toFixed(1)} km del local.`,
            }
          : {
              status: "invalid",
              message: `La dirección está a ${distance.toFixed(1)} km. El delivery llega hasta ${radius} km; podés elegir retiro.`,
            },
      );
    } catch {
      setVerification({
        status: "invalid",
        message:
          "No pudimos verificar la dirección ahora. Intentá nuevamente o elegí retiro.",
      });
    }
  };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        ...
        caught.message.includes("DELIVERY_OUT_OF_RANGE")
      ...
      >
        ...
        <div className="space-y-5 p-5">
          ...
          {customer.fulfillment === "pickup" && (
            <div className="flex items-start gap-3 rounded-xl border border-mustard/40 bg-mustard/10 p-4">
              <MapPin className="mt-0.5 shrink-0 text-mustard" size={20} />
              <div>
          ...
          {customer.fulfillment === "delivery" && (
            <div>
              <Field
                label="Dirección (calle, altura y localidad)"
                value={customer.address}
                onChange={(value) => change("address", value)}
                required
              />
              <p className="mt-1 font-sans text-xs text-cream/65">
                Ejemplo: París 1725 Isidro Casanova. La coma es opcional. Radio
                máximo: {radius} km.
              </p>
              <button
                type="button"
                ...
                disabled={
                  verification.status === "checking" || !customer.address.trim()
                }
                className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-mustard/50 px-4 py-2.5 text-sm font-bold text-mustard disabled:opacity-50"
              >
              ...
              {verification.message && (
                <div
                  role="status"
                  className={`mt-3 flex items-start gap-2 rounded-xl p-3 font-sans text-sm ${verification.status === "valid" ? "bg-green-950/35 text-green-100" : verification.status === "invalid" ? "bg-red-950/35 text-red-100" : "bg-white/5 text-cream/75"}`}
                >
                  {verification.status === "valid" ? (
          ...
          <button
            ...
            className="w-full rounded-xl bg-ember px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
