...
export default function DeliveryCheckout({ cart, settings, onClose, onSuccess }: { cart: CartItem[]; settings: Settings; onClose: () => void; onSuccess: () => void }) {
  ...
  const verifyAddress = async () => {
    setError('')
    const [street, ...localityParts] = customer.address.split(',').map((part) => part.trim()).filter(Boolean)
    const locality = localityParts.join(', ')
    if (!street || !locality) {
      setVerification({ status: 'invalid', message: 'Ingresá calle y altura, seguida de la localidad. Ejemplo: París 1725, Isidro Casanova.' })
      return
    }
